...
void GANNS::SearchTopKonDevice(
  float* queries, int num_of_topk, int*& results, int num_of_query_points, int num_of_candidates)
{
  return graph_->SearchTopKonDevice(
    queries, num_of_topk, results, num_of_query_points, num_of_candidates);
}
...
{
  if (graph_type == "nsw") {
    graph_ = new NavigableSmallWorldGraphWithFixedDegree<metric_type, DIM, collect_metrics>(points);
...
void GANNS::FreeResults(int* results) { graph_->FreeResults(results); }
void GANNS::PrepareDeviceData() {
  graph_->PrepareDeviceData();
}
}  // namespace ganns
