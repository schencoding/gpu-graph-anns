                 ...
                 const nlohmann::json& conf) -> std::unique_ptr<cuvs::bench::algo<T>>
{
  ...
  if constexpr (std::is_same_v<T, float>) {
    if (algo_name == "hnswlib" || algo_name == "hnswlib_32c" || algo_name == "hnswlib_40c") { a = make_algo<T, cuvs::bench::hnsw_lib>(metric, dim, conf); }
  }
...
{
  if (algo_name == "hnswlib" || algo_name == "hnswlib_32c" || "hnswlib_40c") {
